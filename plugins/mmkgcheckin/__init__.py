# -*- coding: utf-8 -*-
"""
MMKG公共节点站签到 (MmkgCheckIn) - MMKG 公共节点站多账号每日签到插件

站点：https://api.mmkg.cloud （New API fork，认证：登录获取 session cookie + New-Api-User 头=用户ID）
流程：登录 -> 拉取站点参数(/api/status) -> 余额(/api/user/self) -> 签到(/api/user/checkin) -> 再查余额 -> 日志补算(/api/log/self)
币种：站点显示模式为 CUSTOM「🐱 咪币」，quota_per_unit=500000，1 咪币 = 500000 quota
记录字段：时间 / 账号 / 状态(success|already|failed) / 本次获得(quota 与 咪币) / 签到后余额 / 说明
"""
import re
...
class MmkgCheckIn(_PluginBase):
    ...
    plugin_icon = "https://raw.githubusercontent.com/wgl520ly/moviepilot-plugins/main/icons/MmkgCheckIn.png"
    ...
    _base_url: str = "https://api.mmkg.cloud"
    _accounts: str = ""
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._base_url = str(config.get("base_url") or "https://api.mmkg.cloud").strip().rstrip("/")
            self._accounts = str(config.get("accounts") or "")

        if not self._enabled:
            logger.info("[MmkgCheckIn] 插件未启用")
            return

        logger.info(f"[MmkgCheckIn] 插件已启用，每日 {self._cron} 执行，站点 {self._base_url}")
        ...
        if self._onlyonce:
            ...
            logger.info("[MmkgCheckIn] 3 秒后立即执行一次")
    ...
    def stop_service(self):
        try:
            ...
            logger.error(f"[MmkgCheckIn] 停止服务失败: {e}")
                ...
                "content": [
                    {
                        ...
                        "content": [
                            ...
                            },
                            {
                                ...
                                "content": [{
                                    "component": "VTextField",
                                    "props": {"model": "base_url", "label": "站点地址", "placeholder": "https://api.mmkg.cloud"}
                                }]
                            },
                            {
                                "component": "VCol",
    ...
    def run_all(self):
        ...
        if not accounts:
            logger.warn("[MmkgCheckIn] 未配置账号，跳过")
            return
        ...
        logger.info(f"[MmkgCheckIn] 签到完成：成功 {ok}，已签到 {already}，失败 {failed}")
    ...
    def __checkin_one(self, acc: dict) -> Tuple[dict, dict]:
        ...
        # 站点认证：登录后保持 session cookie，所有请求带 New-Api-User: 用户ID 头
        sess = requests.Session()
        sess.headers.update({"User-Agent": "MoviePilot/MMKGCheckIn", "Content-Type": "application/json"})

        def req(method: str, path: str, headers: dict = None, json_body: dict = None):
            hdrs = dict(headers or {})
            try:
                resp = sess.request(method, base + path, headers=hdrs, json=json_body, timeout=25)
                try:
        ...
        if not user_id:
            return {"status": "failed", "added_quota": 0, "quota_after": 0, "used_quota": 0,
                    "message": "登录成功但未返回用户ID"}, meta
        auth_headers = {"New-Api-User": str(user_id)}

        try:
            _, status_data = req("GET", "/api/status", headers=auth_headers)
            if isinstance(status_data, dict) and status_data.get("success"):
                ...
                meta = {
                    ...
                    "quota_display_type": str(sd.get("quota_display_type") or "CUSTOM"),
                    "usd_exchange_rate": float(sd.get("usd_exchange_rate") or 1),
                    "custom_currency_symbol": str(sd.get("custom_currency_symbol") or "🐱 咪币 "),
                    "custom_currency_exchange_rate": float(sd.get("custom_currency_exchange_rate") or 1),
                }
        except Exception as e:
            logger.warn(f"[MmkgCheckIn] 获取站点配额参数失败: {e}")

        qpu = int(meta.get("quota_per_unit") or 500000)

        def fetch_self():
            _, d = req("GET", "/api/user/self", headers=auth_headers)
        ...
        def fetch_today_checkin_quota():
            # 站点日志补算当天签到获得的额度（type=4，内容形如「用户签到，获得额度 🐱 咪币 150.000000 额度」）
            try:
                _, ld = req("GET", "/api/log/self?p=1&page_size=50", headers=auth_headers)
                if isinstance(ld, dict) and ld.get("success"):
                    ...
                    for it in items:
                        if it.get("type") == 4 and int(it.get("created_at") or 0) >= day_start_ts:
                            m = re.search(r"获得额度\s*[🐱]?\s*[咪币币＄$¥]?\s*([\d,]+(?:\.[\d]+)?)", str(it.get("content") or ""))
                            if m:
                                try:
                                    ...
                                    pass
                    if total:
                        return int(round(total * qpu))
            except Exception as e:
                logger.warn(f"[MmkgCheckIn] 获取今日签到日志失败: {e}")
            return 0

        quota_before, used_before, _ = fetch_self()

        _, check_data = req("POST", "/api/user/checkin", headers=auth_headers)
        msg = str(check_data.get("message") or "签到失败") if isinstance(check_data, dict) else "签到失败"
        awarded_quota = 0
        if isinstance(check_data, dict) and check_data.get("success"):
            cd = check_data.get("data") or {}
            try:
                awarded_quota = int(round(float(cd.get("quota_awarded") or 0) * qpu))
            except (TypeError, ValueError):
                awarded_quota = 0

        quota_after, used_after, _ = fetch_self()

        today_log_quota = fetch_today_checkin_quota()
        if awarded_quota > 0:
            added = awarded_quota
            status = "success"
        elif today_log_quota > 0:
            # 本次接口返回 0，但当天日志里有实际获得（说明之前已签过）
            added = today_log_quota
            status = "already"
            msg = f"今日已签到，今日已获得 {self.__fmt_money(added, qpu, meta)}"
        else:
            added = 0
            status = "success"
            msg = f"签到成功（本次获得 {self.__fmt_money(0, qpu, meta)}）"

        return {"status": status, "added_quota": added, "quota_after": quota_after or 0,
    ...
    def __parse_accounts(self) -> List[Dict[str, str]]:
        ...
        for line in self._accounts.splitlines():
            ...
            if "|" in line:
                ...
                logger.warn(f"[MmkgCheckIn] 账号行格式不正确，跳过: {line[:20]}")
                continue
    ...
    def __send_notify(self, results: List[dict]):
        ...
        title = f"📊 MMKG公共节点站签到 · {today}" if ok_count else "🔴 MMKG公共节点站签到失败"
        self.post_message(
    ...
    def __fmt_money(value, quota_per_unit: int, meta: dict) -> str:
        ...
        if d_type == "CNY":
            ...
            symbol = str((meta or {}).get("custom_currency_symbol") or "🐱 咪币 ")
            try:
